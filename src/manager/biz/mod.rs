...
impl CategoryBiz {
    ...
    fn internal(e: impl ToString) -> Status { Status::internal(e.to_string()) }
    ...
    async fn assert_member(&self, budget_id: &str, user_id: &str) -> Result<(), Status> {
        let role = self.budget_client.lock().await
            .check_role(user_id, budget_id).await?;
    ...
    pub async fn create_category(
        ...
        let db = self.repo
            .create_category(budget_id, name, cat_type, icon, color, planned_amount, user_id)
            .await.map_err(Self::internal)?;
        Ok(map_category(db))
    }
    ...
    pub async fn update_category(
        ...
        let budget_id = self.repo.get_budget_id(category_id).await.map_err(Self::internal)?
    ...
    pub async fn archive_category(&self, user_id: &str, category_id: &str) -> Result<(), Status> {
        let budget_id = self.repo.get_budget_id(category_id).await.map_err(Self::internal)?
            .ok_or_else(|| Status::not_found("Category not found"))?;
        self.assert_manager(&budget_id, user_id).await?;
        self.repo.archive_category(category_id).await.map_err(Self::internal)
    }
    ...
    pub async fn delete_category(&self, user_id: &str, category_id: &str) -> Result<(), Status> {
        let budget_id = self.repo.get_budget_id(category_id).await.map_err(Self::internal)?
            .ok_or_else(|| Status::not_found("Category not found"))?;
        self.assert_manager(&budget_id, user_id).await?;
        self.repo.delete_category(category_id).await.map_err(Self::internal)
    }

    pub async fn get_category(&self, user_id: &str, category_id: &str) -> Result<Category, Status> {
        let db = self.repo.get_category(category_id).await
            .map_err(|_| Status::not_found("Category not found"))?;
        self.assert_member(&db.budget_id, user_id).await?;
        Ok(map_category(db))
    }

    pub async fn list_categories(&self, user_id: &str, budget_id: &str) -> Result<Vec<Category>, Status> {
        self.assert_member(budget_id, user_id).await?;
        let rows = self.repo.list_categories(budget_id).await.map_err(Self::internal)?;
